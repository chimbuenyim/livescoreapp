...
<div class="match-players-details">
    <span class="details details-home">
        <span class="details-player">${(event.score && event.home_scorer) ? `Goal!!! ${event.home_scorer}` : (event.card && event.home_fault) ? `${findImage(event.card)} ${event.home_fault}`:''}</span>
    </span>
    <span class = "details">
        <span class="details-center">${event.score && event.score.split(' - ')[0] || ''}</span>
        <span class="dash">${event.score && '-' || ''}</span>
        <span class="away-score">${event.score && event.score.split(' - ')[1] || ''}</span>
    </span>
    <span class="details details-away">
        <span class="details-player">${(event.score && event.away_scorer) ? `Goal!!! ${event.away_scorer}` : (event.card && event.away_fault) ? `${findImage(event.card)} ${event.away_fault}`:''}</span>
    </span>
</div>
...
function displayStandings(table){
    let standings_table = document.querySelector('.league-table-body')
